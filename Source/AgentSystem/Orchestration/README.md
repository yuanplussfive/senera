...
`AgentSpawn` durably queues one child and always returns immediately. Every child also has a logical `workItemId` and task-contract digest, separate from its ephemeral execution `runId`. Reusing the same work-item identity within a parent session converges concurrent or retried submissions on the existing child; a different identity is required for intentionally repeated work. `AgentList` recovers the durable run projection in a later turn, including progress, terminal results, and result/wake-consumption state. Multiple spawn calls in one native Tool batch initialize independently and then run under provider and Tool-resource backpressure. The parent can keep doing non-overlapping work instead of blocking on child completion.
