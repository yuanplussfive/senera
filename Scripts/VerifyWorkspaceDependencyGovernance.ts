...
function inspectSecurityScanWorkflow(): string[] {
  return [
    ...inspectTextIncludes(securityScanWorkflow, ".github/workflows/security-scan.yml", [
      "name: Security Scan",
    ...
    ...inspectPullRequestJobGate(securityScanWorkflow, ".github/workflows/security-scan.yml", "codeql"),
    ...inspectPullRequestJobGate(securityScanWorkflow, ".github/workflows/security-scan.yml", "trivy-filesystem"),
  ];
}

function inspectPullRequestJobGate(workflow: string, file: string, jobName: string): string[] {
  const block = readWorkflowJob(workflow, jobName);
  if (!block) {
    return [`${file} must define the ${jobName} job.`];
  }
...
function readWorkflowJob(workflow: string, jobName: string): string | undefined {
