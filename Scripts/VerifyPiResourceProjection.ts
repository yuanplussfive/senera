...
fs.writeFileSync(path.join(pluginRoot, "skills", "Workspace.md"), "# WorkspaceSkill\n\nUse workspace evidence.", "utf8");
...
assert.deepEqual(resources.harnessResources.skills?.map((skill) => skill.name), ["WorkspaceSkill"]);
...
assert.deepEqual(resources.harnessResources.promptTemplates?.map((template) => template.name), ["VisiblePiTemplate"]);
...
assert.deepEqual(resources.selection.promptTemplates.map((selection) => selection.template.name), ["VisiblePiTemplate"]);
