...
-- table: roleplay_state_events
create table roleplay_state_events ( id text primary key , session_id text not null , request_id text not null , revision integer not null check ( revision > 0 ) , event_index integer not null check ( event_index >= 0 ) , created_at text not null , event_json text not null , unique ( session_id , revision , event_index ) , foreign key ( session_id ) references sessions ( id ) on delete cascade );

-- table: roleplay_state_snapshots
create table roleplay_state_snapshots ( session_id text primary key , revision integer not null check ( revision >= 0 ) , updated_at text not null , state_json text not null , foreign key ( session_id ) references sessions ( id ) on delete cascade );
...
-- index: idx_roleplay_state_events_session_request
create index idx_roleplay_state_events_session_request on roleplay_state_events ( session_id , request_id );

-- index: idx_roleplay_state_events_session_revision
create index idx_roleplay_state_events_session_revision on roleplay_state_events ( session_id , revision , event_index );
