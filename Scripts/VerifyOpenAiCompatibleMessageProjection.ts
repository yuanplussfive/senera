...
assert.deepEqual(systemCompatible.map((message) => message.role), ["system", "user"]);
...
assert.deepEqual(nativeDeveloper.map((message) => message.role), ["system", "developer", "user"]);
assert.equal(nativeDeveloper[0]?.content, "runtime system");
assert.equal(nativeDeveloper[1]?.content, "developer rule");

const piProvider = projectSeneraModelProviderToPi(
  createProvider({ DeveloperRole: false }),
  createConfig(),
);
assert.equal(piProvider.model.compat?.supportsDeveloperRole, false);

const piDeveloperProvider = projectSeneraModelProviderToPi(
  createProvider({ DeveloperRole: true }),
  createConfig(),
);
...
async function verifyOpenAiEndpointPayloadProjection(): Promise<void> {
  ...
  const chatEndpoint = new OpenAiChatCompletionsEndpoint(createRuntime(
    createProvider({ DeveloperRole: false }),
    chatHttp,
  ));
  await chatEndpoint.complete(request);
  assert.deepEqual(
    readMessageRoles(chatHttp.lastJsonPayload?.messages),
    ["system", "user"],
  );

  const chatDeveloperHttp = new RecordingHttpClient({ choices: [{ message: { content: "ok" } }] });
  const chatDeveloperEndpoint = new OpenAiChatCompletionsEndpoint(createRuntime(
    createProvider({ DeveloperRole: true }),
    chatDeveloperHttp,
  ));
  await chatDeveloperEndpoint.complete(request);
  assert.deepEqual(
    readMessageRoles(chatDeveloperHttp.lastJsonPayload?.messages),
    ["system", "developer", "user"],
  );

  const responsesHttp = new RecordingHttpClient({ output_text: "ok" });
  const responsesEndpoint = new OpenAiResponsesEndpoint(createRuntime(
    createProvider({ DeveloperRole: false, Chat: true }),
    responsesHttp,
  ));
  await responsesEndpoint.complete(request);
  assert.deepEqual(
    readMessageRoles(responsesHttp.lastJsonPayload?.input),
    ["system", "user"],
  );

  const responsesDeveloperHttp = new RecordingHttpClient({ output_text: "ok" });
  const responsesDeveloperEndpoint = new OpenAiResponsesEndpoint(createRuntime(
    createProvider({ DeveloperRole: true, Chat: true }),
    responsesDeveloperHttp,
  ));
  await responsesDeveloperEndpoint.complete(request);
  assert.deepEqual(
    readMessageRoles(responsesDeveloperHttp.lastJsonPayload?.input),
    ["system", "developer", "user"],
  );
}
