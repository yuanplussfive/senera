...
function inspectRootScripts(): string[] {
  const expectedScripts = {
    ...
    build:
      "npm run verify.config-command-contracts && npm run verify.database-contracts && npm run verify.frontend-events && npm run verify.protocol-reference && npm run clean && tsc && tsx Build/CopyRuntimeAssets.ts",
...
function inspectReleaseWorkflowGates(): string[] {
  return [
    ...inspectTextIncludes(productReleaseWorkflow, ".github/workflows/release.yml", [
      ...
      "Rerun Verify for the release commit before recovering its tag.",
