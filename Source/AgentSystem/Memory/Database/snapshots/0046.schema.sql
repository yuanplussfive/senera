...
-- table: agent_agenda_command_receipts
create table agent_agenda_command_receipts ( command_id text primary key , operation_kind text not null , payload_hash text not null , record_id text not null , event_id text not null unique , revision integer not null check ( revision > 0 ) , created_at text not null , foreign key ( record_id ) references agent_agenda_records ( id ) on delete cascade , foreign key ( event_id ) references agent_agenda_events ( id ) on delete cascade );
...
create table agent_world_events ( id text primary key , uri text not null unique , world_id text not null , sequence integer not null check ( sequence > 0 ) , idempotency_key text not null unique , subject_id text not null , subject_kind text not null , event_type text not null , summary text not null , changes_json text not null , evidence_refs_json text not null , occurred_at text not null , recorded_at text not null , local_date text not null , summary_parts_json text not null default '[]' , foreign key ( world_id ) references agent_agenda_worlds ( id ) on delete cascade , unique ( world_id , sequence ) ) strict;
...
-- table: agent_world_work_items
create table agent_world_work_items ( id text primary key , world_id text not null , source_id text not null , candidate_id text not null , request_id text not null unique , payload_hash text not null , payload_json text not null , status text not null check ( status in ( 'pending' , 'leased' , 'running' , 'acknowledged' , 'failed' , 'unknown' , 'cancelled' , 'reconciliation_required' ) ) , lease_owner text , lease_generation integer not null default 0 check ( lease_generation >= 0 ) , lease_until text , attempt_count integer not null default 0 check ( attempt_count >= 0 ) , next_attempt_at text not null , started_at text , completed_at text , result_json text , evidence_refs_json text not null default '[]' , last_error text , created_at text not null , updated_at text not null , foreign key ( world_id ) references agent_agenda_worlds ( id ) on delete cascade , unique ( world_id , source_id , candidate_id ) , check ( ( status in ( 'leased' , 'running' ) and lease_owner is not null and lease_until is not null ) or ( status not in ( 'leased' , 'running' ) and lease_owner is null and lease_until is null ) ) , check ( ( status in ( 'acknowledged' , 'unknown' , 'cancelled' , 'reconciliation_required' ) and completed_at is not null ) or ( status not in ( 'acknowledged' , 'unknown' , 'cancelled' , 'reconciliation_required' ) ) ) ) strict;
...
-- table: continuity_learning_inferences
create table continuity_learning_inferences ( inference_key text primary key , stage text not null check ( stage in ( 'facts' , 'rules' ) ) , contract_revision text not null , bundle_revision text not null , provider_id text not null , model text not null , input_json text not null check ( json_valid ( input_json ) ) , output_json text not null check ( json_valid ( output_json ) ) , feature_keys_json text not null check ( json_valid ( feature_keys_json ) and json_type ( feature_keys_json ) = 'array' ) , accepted_item_count integer not null check ( accepted_item_count >= 0 ) , source_episode_uri text not null , created_at text not null , created_at_ms integer not null , last_used_at text not null , last_used_at_ms integer not null , use_count integer not null check ( use_count >= 1 ) , foreign key ( source_episode_uri ) references memory_episodes ( uri ) on delete cascade );
...
-- index: idx_agent_agenda_command_receipts_record
create index idx_agent_agenda_command_receipts_record on agent_agenda_command_receipts ( record_id , revision );
...
-- index: idx_agent_world_work_items_due
create index idx_agent_world_work_items_due on agent_world_work_items ( world_id , status , next_attempt_at , created_at , id );

-- index: idx_agent_world_work_items_lease
create index idx_agent_world_work_items_lease on agent_world_work_items ( status , lease_until , updated_at );

-- index: idx_agent_world_work_items_source
create index idx_agent_world_work_items_source on agent_world_work_items ( world_id , source_id , status , updated_at );
...
-- index: idx_continuity_learning_inferences_examples
create index idx_continuity_learning_inferences_examples on continuity_learning_inferences ( stage , contract_revision , accepted_item_count desc , last_used_at_ms desc );

-- index: idx_continuity_learning_inferences_source
create index idx_continuity_learning_inferences_source on continuity_learning_inferences ( source_episode_uri );
