...
describe("resolveConfigSettingsEvent", () => {
    it("resolves pending config update snapshots as success", () => {
        expect(resolveConfigSettingsEvent(event(EventKinds.ConfigSnapshot, {
            path: "Config.toml",
            version: 2,
            value: {},
            source: "sqlite",
    ...
    it("ignores non-pending config snapshots", () => {
        expect(resolveConfigSettingsEvent(event(EventKinds.ConfigSnapshot, {
            ...
            diagnostics: [],
            form: { version: 1, sections: [] },
        }), new Set(["req-1"]))).toBeNull();
    });
    it("resolves config update failures before plugin fallback handles config.failed", () => {
        expect(resolveConfigSettingsEvent(event(EventKinds.ConfigFailed, {
    ...
    it("resolves provider model snapshots and failures as loading completion", () => {
        expect(resolveConfigSettingsEvent(event(EventKinds.ProviderModelsSnapshot, {
            providerId: "openai",
        ...
        expect(resolveConfigSettingsEvent(event(EventKinds.ProviderModelsFailed, {
            providerId: "openai",
