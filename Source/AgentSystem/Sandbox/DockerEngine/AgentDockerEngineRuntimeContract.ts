...
export const AgentSandboxRuntimeContractFormatVersion = 2 as const;
  ...
  .object({
    ...
    guest: z
      .object({
        ...
        terminal: z
          .object({ command: PosixAbsolutePathSchema, arguments: z.array(z.string()).min(1).max(16) })
          .strict(),
...
export function readAgentDockerEngineRuntimePolicyContract(
  ...
  const commonRelativePath = "contract.json";
  const common = AgentDockerEngineCommonPolicySchema.parse(
    parseJsonText(
      ...
      `Docker Engine runtime contract: ${commonRelativePath}`,
    ),
  );
  const overlayRelativePath = provider === "gvisor" ? "Gvisor/contract.json" : "Default/contract.json";
  const overlay = AgentDockerEngineProviderOverlaySchema.parse(
    parseJsonText(
      ...
      `Docker Engine runtime contract: ${overlayRelativePath}`,
    ),
  );
