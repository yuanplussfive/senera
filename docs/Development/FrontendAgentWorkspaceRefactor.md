...
状态：方案 B 已选定；本文保留历史方案与实施记录，当前实现约束见第 23 节
...
当前 `AppShell` 的 Workflow 执行面板宽度为 `302px`，桌面端在 `1024px` 起常驻工具坞，`1280px` 起进入中央 Flex 布局；小屏使用 Drawer。目标不是删除 Workflow，而是保持它作为按需打开的执行视图。
...
- Desktop/Wide 展开：`246px`。
- 收起：`58px` 紧凑侧栏，不额外保留第二条 rail。
...
当前主壳层实际仍由 `AppShell` 承担；这些名称不是当前必须存在的组件契约。
...
- Desktop/Wide 左侧改为单一整合侧栏；展开为 `246px`，收起为 `58px`，不保留第二条 rail。
- 增加真实会话搜索，并保留新建、选择、重命名、删除、账户和设置行为。
- 右侧拆分为常驻执行工具坞与按需执行面板；新用户默认收起执行面板。
- 1024–1279px 使用覆盖面板，1280px 起并列参与中央 Flex 伸缩；Workflow 面板宽度统一为 `302px`，1536px 只改变 viewport 分类，不再扩大为 460px。
...
## 23. 当前清理与后续能力边界（2026-07-19）

本轮清理以生产调用链和当前组件状态为准：

- 已删除旧 App 适配层、旧配置 mutation 链、旧 `SystemConfigPanel` 模型页面、模型分组编辑器、过时 shadcn CLI 配置、重复 JsonConfig 拆分模块和只转导出的兼容文件。
- 模型分组仅保留当前模型列表需要的只读解析、默认分组和展示筛选；完整分组规则编辑以后作为独立能力重新设计。
- `PlanningConfigView.tsx` 与 `VectorModelConfigView.tsx` 暂时保留，作为后续 Planner 模型选择和 Embedding/Rerank 专用选择迁移到 `SettingsWorkbench` 的能力参考；当前 Settings 页面仍使用通用 JSON 配置表单。
- `shared/ui/ErrorBoundary.tsx` 同时承担组件级和根节点级恢复，`AppErrorBoundary.tsx` 不再单独存在。
- Workflow 工具坞保留执行入口和终端占位标签；终端当前只展示占位 Surface，未来能力必须在有真实 Surface 后再接入。

## 24. 公共值选择菜单基线（2026-07-19）

已确认的视觉基线是当前生产方案。`MenuSelect` 现在是模型、Planner、Embedding/Rerank、默认模型和供应商预设选择共用的值选择组件。

- `MenuSelect` 负责紧凑触发器、键盘焦点、禁用态、选项渲染和空状态。
- `DropdownMenu` 继续负责包含标题、分隔线、危险操作或快捷键的动作菜单。
- 默认值选择菜单不增加重复标题、分隔线、选中勾或装饰图标。
- 供应商和模型图标属于语义数据，必须由调用页面通过渲染属性明确传入。
- 菜单内容宽度不强制等于触发器宽度；较长的模型标识应保持可读，选项面板可以自然加宽。
- 模型分组编辑明确不属于本轮迁移范围，未来作为独立能力重新设计。

## 25. 公共布尔开关规范（2026-07-19）

本轮按当前生产组件状态收敛布尔开关。以后凡是“启用/停用、允许/禁止、显示/隐藏”这类二值设置，都应优先使用 `Frontend/src/shared/ui/Switch.tsx` 导出的 `Switch`。

- `Switch` 负责开关轨道、滑块、主题强调色、禁用态、键盘焦点、`role="switch"`、`aria-checked` 和短促状态动画。
- 焦点只保留 1px 的细边界，不使用 2px 彩色光圈；状态文字由页面需要决定，不能为了显示“已启用/已关闭”重复占位。
- `Switch` 只渲染轨道和滑块，不内置文字。`checked`、`ariaLabel` 和 `onCheckedChange` 是必需契约；需要说明时，由业务外壳在旁边放普通文字。
- 默认开启色使用当前主题的强调色。moss/绿色只表达成功、已连接、健康或可用状态，不能再作为所有开启开关的默认颜色。
- `SwitchTrack` 只用于外层已经是按钮的复合控件，例如整张工具卡或整行选择项。它本身不接收输入，也不携带新的无障碍名称，避免按钮嵌套。
- 业务组件可以保留自己的标题、说明、图标和排列方式，但不能重新绘制另一套轨道、滑块或局部开关动画。
- 模型能力、模型配置、插件配置、供应商连接、向量模型配置、JSON 设置和外观设置中的二值控件已迁移到这套基线。向量模型页原来的 `ON/OFF + RefreshCw` 组合已改为真正的开关；刷新仍是独立动作时才使用刷新按钮。
- 模型分组编辑不属于本轮范围，旧编辑器已经删除，未来作为独立能力重新设计。

菜单选择和动作菜单不属于开关：值选择继续使用 `MenuSelect`，带删除、分隔线或快捷键的动作集合继续使用 `DropdownMenu`。不要用一个“万能菜单”或“万能开关”覆盖不同交互语义。

## 26. Agent 与 Story 同步硬性规则（2026-07-19）

从现在开始，Agent 修改真实公共组件时，必须在同一次变更中检查并同步对应 Story。这里的“同步”不是把组件 CSS 复制进 Story，而是让 Story 继续直接引用真实组件，并反映修改后的真实状态、属性、交互和可访问性契约。

- 修改 Frontend/src/shared/ui 中任何公共组件的样式、属性、状态、交互或无障碍行为，都必须检查对应的 ComponentName.stories.tsx。
- 如果视觉或交互契约发生变化，必须同时调整 Story 的示例、状态或说明；不能只改生产组件而留下过时示例。
- 新增公共视觉组件时，必须同时完成 shared/ui/index.ts 导出和同名 Story。
- Agent 完成修改前必须运行 npm --workspace senera-frontend run check.ladle；涉及主题、src/index.css 或公共组件样式时，还必须运行 npm --workspace senera-frontend run ladle:build。
- check.ladle 会验证公共导出、同名 Story、真实组件直接导入、中文文案和开关文字约束；它不能替代 Agent 对 Story 是否真实覆盖新状态的判断。
- 这条规则同时适用于人工修改和 Agent 修改，后续新增的公共组件自动进入同一套门禁。

## 27. 之前需求的当前状态（2026-07-19）

### 已完成

1. 旧 App 适配层、旧配置 mutation 链、模型分组编辑器、过时 shadcn CLI 配置、重复 JsonConfig 拆分模块和兼容转导文件已删除。
2. MenuSelect 已成为值选择公共组件；DropdownMenu 和 ContextMenu 继续承担动作菜单，并共用 MenuShared.tsx 的菜单表面和菜单项样式。
3. 公共 Switch 已收敛为纯轨道与滑块，去除开关旁重复状态文字和过度的彩色焦点光圈。
4. kill / emil 讨论后的当前方案已确认，并同步到公共组件、文档和 Ladle。
5. Ladle 已补齐公共视觉组件 Story，统一中文文案，并接入自动契约检查。
6. 已完成 Ladle 结构门禁、类型检查、构建和自动化测试；真实浏览器视觉复核仍需在可访问宿主机 localhost 的 Electron 或浏览器会话中按 `390px`、`900px`、`1280px`、`1440px` 和 `1600px` 五档完成。

### 明确暂缓

- Planner 模型选择、Embedding/Rerank 向量模型选择会保留现有参考实现，后续单独设计；模型分组编辑不在本轮范围。

### 当前状态

- Ladle 约束、Story 同步规则和自动契约检查已完成；真实页面复核保留为下一步验证项。
- `ChatComposer` 是依赖运行时上传、模型和预设状态的业务组件，目前没有独立 Ladle Story；本轮没有为了展示而伪造运行时数据。它的 `composer` 内容轴线已通过 `ConversationFrame` Story 和现有表单、抽屉组合复核。若后续要单独调整 Composer 的业务布局，应先补充真实运行时边界，再新增对应 Story。