...
-- table: agent_goal_evidence
create table agent_goal_evidence ( id text primary key , uri text not null unique , goal_id text not null , session_id text not null , request_id text not null , kind text not null check ( kind in ( 'tool' , 'artifact' , 'execution' , 'gate' ) ) , source_uri text not null , label text not null , observed_at text not null , unique ( goal_id , source_uri ) , foreign key ( goal_id ) references agent_goals ( id ) on delete cascade );
...
-- index: idx_agent_goal_evidence_goal_time
create index idx_agent_goal_evidence_goal_time on agent_goal_evidence ( goal_id , observed_at , id );

-- index: idx_agent_goal_evidence_request
create index idx_agent_goal_evidence_request on agent_goal_evidence ( session_id , request_id , observed_at );
