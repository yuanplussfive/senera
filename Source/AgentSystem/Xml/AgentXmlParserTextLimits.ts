...
import { AgentXmlParseError, type AgentXmlParserOptions } from "./AgentXmlParserTypes.js";

export function assertXmlParserTextLimits(trimmed: string, options: AgentXmlParserOptions): void {
  if (!trimmed) {
  ...
  if (tokenBudget?.state === "limit_reached") {
    throw new AgentXmlParseError(
      "XML 输出超过最大 token 限制。",
      [
        new AgentSourceDiagnosticBuilder(trimmed).fromPosition("XML 输出超过最大 token 限制。", 0, {
          pointer: "/",
