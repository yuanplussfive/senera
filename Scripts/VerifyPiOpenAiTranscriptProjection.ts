...
import { entryToRow, rowToEntry } from "../Source/AgentSystem/SessionPersistence/AgentConversationEntryCodec.js";
...
function verifyOpenAiTranscriptPersistenceRoundTrip(): void {
  ...
  const entry = conversation.projectOpenAiTranscript("verify-transcript", messages, "2026-01-01T00:00:00.000Z");
  ...
  assert.deepEqual(restored?.kind === AgentConversationEntryKinds.OpenAiTranscript ? restored.messages : [], messages);
}
...
function verifyPiHistoryUsesCanonicalTranscript(): void {
  ...
  const projected = projector.project({
    ...
    userInput: "继续总结",
    conversationEntries: [
      conversation.projectOpenAiTranscript("previous-request", transcriptFixture(), "2026-01-01T00:00:00.000Z"),
      conversation.projectUserInput("current-request", "继续总结", "2026-01-01T00:00:02.000Z"),
    ],
    model: modelFixture(),
  });
  ...
  assert.deepEqual(
    projected.history.map((message) => message.role),
...
function expectAssistantText(message: { content?: Array<Record<string, unknown>> }, text: string): void {
  assert.equal(
