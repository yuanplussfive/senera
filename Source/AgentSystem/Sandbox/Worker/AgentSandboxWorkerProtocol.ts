...
export const AgentSandboxWorkerProtocolVersion = 3;
const RuntimePolicy = readAgentDockerEngineRuntimePolicyContract("docker-engine");
export const AgentSandboxWorkerMaxFrameBytes = RuntimePolicy.protocol.maxFrameBytes;

const NonEmptyString = z.string().trim().min(1);
const AbsoluteGuestPath = z.string().startsWith("/").min(1);
const InitialMessage = z.object({ protocolVersion: z.literal(AgentSandboxWorkerProtocolVersion) });
const PreparationStage = z.enum(Object.values(AgentSandboxPreparationStages));
const ProgressCount = z.number().int().nonnegative();

export const AgentSandboxExecutionRequestSchema = z
  .object({
...
export type AgentSandboxExecutionRequest = z.infer<typeof AgentSandboxExecutionRequestSchema>;

export const AgentSandboxWorkerClientMessageSchema = z.discriminatedUnion("type", [
  ...
  InitialMessage.extend({
    ...
    request: AgentSandboxExecutionRequestSchema,
  }).strict(),
  z
    .object({
...
export type AgentSandboxWorkerClientMessage = z.infer<typeof AgentSandboxWorkerClientMessageSchema>;

export const AgentSandboxWorkerServerMessageSchema = z.discriminatedUnion("type", [
  z
    .object({
...
export type AgentSandboxWorkerServerMessage = z.infer<typeof AgentSandboxWorkerServerMessageSchema>;
