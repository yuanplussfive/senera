...
- `AgentToolExecutionScheduler`：按每次运行容量、工具级上限和显式调度模式协调普通工具调用。
- `AgentToolResourceClaimProjector` / `AgentToolResourceScheduler`：只为 `ResourceClaims` 工具投影运行时租约，并按资源关系协调冲突。
...
- ToolRuntime 不负责规划要调用哪些工具；native provider 使用供应商 Tool Calling，BAML provider 通过 planning compiler 与 `ActionPlanner` 编译下一步决策和参数。ToolRuntime 只消费已经过合同校验的调用。
...
- `Runtime.Scheduling=SelfManaged` 只适用于拥有显式领域 permit、取消和关闭语义的编排入口；不能用它给普通工具绕过通用容量控制。
...
- `Execution.Workspace` 不是并发声明。冲突控制来自 `ResourceClaims`，工具级容量来自 `Runtime.MaxConcurrency`；只声明 `ReadOnly` 不能证明网络或外部系统没有副作用。
