...
import {
  ...
  type AgentPiDiagnosticInput,
  type AgentPiDiagnosticEvent,
  type AgentPiDiagnosticSink,
} from "../PiShared/AgentPiDiagnosticsTypes.js";
...
export async function emitAgentPiDiagnostic(
  ...
  try {
    ...
    // Diagnostics are observational and must never alter agent behavior.
  }
}
 ...
 */
export function createAgentPiDiagnosticLoggerForDetail(
  logger: AgentLogger,
  detail: "compact" | "verbose",
): AgentPiDiagnosticSink | undefined {
  if (detail === "verbose") {
    return createAgentPiDiagnosticLogger(logger);
  }
  return (event) => {
    if (!CriticalDiagnosticNames.has(event.name)) return;
    const label = `pi.${event.source}.${event.name}`;
    logger.warn(label, {
      ...event.context,
      summary: event.summary || undefined,
