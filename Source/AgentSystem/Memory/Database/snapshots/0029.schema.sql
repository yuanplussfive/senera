...
create table agent_execution_runs ( id text primary key , uri text not null unique , session_id text not null , request_id text not null , goal_id text , objective text not null , status text not null check ( status in ( 'active' , 'paused' , 'blocked' , 'completed' , 'cancelled' ) ) , reason text , created_at text not null , updated_at text not null , completed_at text , unique ( session_id , request_id ) , foreign key ( goal_id ) references agent_goals ( id ) on delete set null );
...
-- table: agent_goals
create table agent_goals ( id text primary key , uri text not null unique , scope_kind text not null , scope_id text not null , objective text not null , status text not null check ( status in ( 'active' , 'paused' , 'blocked' , 'completed' , 'cancelled' ) ) , reason text , created_at text not null , updated_at text not null , completed_at text , session_id text , request_id text , origin text not null default 'manual' check ( origin in ( 'manual' , 'automatic' ) ) , contract_json text not null default '{}' , gates_json text not null default '[]' , turn_budget integer not null default 20 , turns_used integer not null default 0 , last_verdict text , last_reason text , paused_reason text , consecutive_judge_failures integer not null default 0 , wait_barrier_json text , continuation_context_json text , wake_claim_id text , wake_claim_until text , wake_request_id text , wake_failures integer not null default 0 check ( wake_failures >= 0 ) , wake_last_error text );
...
create table "continuity_concepts" ( id text primary key , uri text not null unique , canonical_label text not null , normalized_label text not null , scope_kind text not null , scope_id text not null , status text not null check ( status in ( 'active' , 'merged' , 'retired' ) ) , merged_into_uri text , created_at text not null , updated_at text not null );
...
create table "continuity_record_concepts" ( record_uri text not null , record_kind text not null check ( record_kind in ( 'fact' , 'profile' , 'signal' , 'rule' ) ) , concept_uri text not null , linked_at text not null , primary key ( record_uri , record_kind ) , foreign key ( concept_uri ) references "continuity_concepts" ( uri ) on delete cascade );
...
-- table: continuity_trivial_prompt_cache
create table continuity_trivial_prompt_cache ( prompt_hash text primary key , occurrences integer not null check ( occurrences >= 0 ) , first_seen_at text not null , last_seen_at text not null );
...
-- index: idx_agent_goals_request
create index idx_agent_goals_request on agent_goals ( session_id , request_id ) where session_id is not null and request_id is not null;

-- index: idx_agent_goals_scope_status
create index idx_agent_goals_scope_status on agent_goals ( scope_kind , scope_id , status , updated_at );

-- index: idx_agent_goals_session_request
create unique index idx_agent_goals_session_request on agent_goals ( scope_kind , scope_id , request_id ) where request_id is not null;

-- index: idx_agent_goals_wake_claim
create index idx_agent_goals_wake_claim on agent_goals ( status , wake_claim_until , updated_at ) where wait_barrier_json is not null and continuation_context_json is not null;
...
create index idx_continuity_concepts_scope on continuity_concepts ( scope_kind , scope_id , status , updated_at );
...
-- index: idx_continuity_trivial_prompt_cache_last_seen
create index idx_continuity_trivial_prompt_cache_last_seen on continuity_trivial_prompt_cache ( last_seen_at desc );
