...
try {
  ...
  assert.equal(
    imported.value.ModelProviders.some((model) => model.Id === "custom/model-a"),
    true,
  );
  assert.deepEqual(
    imported.value.ModelGroups?.find((group) => group.Id === "reasoning"),
    {
      Id: "reasoning",
      Label: "Reasoning",
      Icon: "brain",
  ...
  assert.equal(
    service.snapshot().value.ModelProviders.some((model) => model.ProviderId === "custom-renamed"),
    true,
  );
  ...
  assert.equal(
    endpointDeleted.value.ModelProviderEndpoints?.some((endpoint) => endpoint.Id === "custom-renamed"),
    false,
  );
  assert.equal(
    endpointDeleted.value.ModelProviders.some((model) => model.ProviderId === "custom-renamed"),
    false,
  );
