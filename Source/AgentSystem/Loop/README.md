...
AgentLoop.run
  ...
  -> AgentTurnPromptRenderer
     -> native: PiNativeSystemPrompt
     -> baml: PiBamlSystemPrompt
  -> AgentPiTurnExecutor
     -> project product conversation
     -> bind session-owned turn state
...
- `AgentTurnPreparationSnapshot`：持久化与运行 fingerprint 绑定的 grant、请求级工具授权 ceiling、工具曝光、RootCommand、活动 Skills 和 Pi branch boundary；配置、合同或 ceiling 变化后不得复用。
- `AgentTurnPromptRenderer`：根据已解析的 `ToolPlanningMode` 选择声明式 prompt profile，渲染本轮 system prompt 并计算 token 数。Native 与 BAML 共享会话事实，但不共享工具编码协议。
...
- BAML provider 中的 `EvolveTurn`、参数物化和 repair 驱动 Pi 内部多步循环；Native provider 直接使用供应商 Tool Calling。Loop 不复制任一模式的决策状态。
- Turn preparation 只建立语义 RootCommand、grant、曝光和 Skill 状态，不把用户输入复制进 RootCommand。Native prompt 不包含 RootCommand、文本工具目录或工具输出哨兵；BAML 的 RootCommand 只通过结构化 `seneraRuntime` 进入 planner。
- `message_end` 产生稳定终态文本时，Loop 使用固定 message ID 先发布 `terminal=false` 的可见回答。Pi 的自动压缩与 session settlement 完成后，同一 message ID 才随 `RunCompleted` 原子提交为 `terminal=true`；前端因此可以显示答案，同时继续展示 `compacting_context`。
- `preferredToolNames ⊆ exposedToolNames ⊆ authorizedToolNames ⊆ registeredToolNames`。存在请求级 `allowedToolNames` 时，`authorizedToolNames` 必须先与该 ceiling 求交；Skill 推荐只在最终 `exposedToolNames` 内形成偏好。Loop 只建立初始集合；同一回合后续曝光由 ToolSearch 和 `AgentToolExposureState` 处理，但不能越过授权 ceiling。
- `commitTerminalEvents` 存在时，Loop 不自行发布终态。Session 必须先原子写入最终对话、run snapshot、trace 和 durable events，再向订阅者发布。
- 取消、超时和异常都必须经过 `finally` 释放 retrieval、清除 session turn state 并释放 Pi session lease；不得引入模块级默认上下文或 fire-and-forget 的关键收尾。
