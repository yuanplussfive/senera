...
const attachments: UploadAttachmentData[] = [{
  uploadUri: "upload://session-a/request-a/file.txt",
  name: "file.txt",
  mime: "text/plain",
  size: 12,
  status: "uploaded",
...
describe("executeSessionTruncateReplay", () => {
  it("sends the replay request before appending the replacement user message", () => {
    ...
    expect(executeSessionTruncateReplay({
      appendUserMessage: (sessionId, requestId, input, nextAttachments) => {
        calls.push("append");
        appended.push({ sessionId, requestId, input, attachments: nextAttachments });
      },
      lastSendRef,
