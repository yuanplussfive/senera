...
| `Dialog`            | 需要确认或集中处理的模态任务 | 标题、说明、底部操作区（`footer`）、普通动作和危险动作 |
...
- 通用开启态使用中性强色（`content-strong`），不跟随主题强调色；原生复选框的开态用同一中性色（`accent-content-strong`）。主题强调色只用于可选中、可交互的强调，绿色只表达成功、连接、健康或可用状态。
...
- 弹窗面板（`DialogContent`）自己负责圆角（`rounded-xl`）、边框（`border-line`）和底色（`surface-panel`）；业务弹窗不要再传 `rounded-*` / `bg-paper-*` 覆盖面板外观。
- 底部操作区通过 `footer` 属性交给面板渲染，面板只负责内边距（上方 16px、底部 14px），不画底色也不画分隔线；`DialogActions` 只负责按钮排布，不承担间距。
- 正文保留自己的下内边距（内容与按钮之间的呼吸），`footerClassName` 只在正文横向内边距不是 24px 时用来对齐（例如 `px-8`）。

## 5. 视觉边界

- 使用真实语义颜色和主题 token，不在 Story 中硬编码另一套品牌色。
- 不新增玻璃模糊、彩色发光、渐变光斑、重复胶囊、无意义卡片和卡片套卡片。
- 普通产品表面圆角保持在 6-10px；浮层弹窗面板（`DialogContent`）使用 12px（`rounded-xl`）；头像、开关、进度轨道和聊天气泡按自身几何语义处理。
