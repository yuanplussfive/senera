...
test("desktop overlay opens from a floating capsule and switches accessible horizontal tabs", async () => {
  ...
  const keyboardSelectedTerminalTab = screen.getByRole("tab", { name: "终端" });
  expect(keyboardSelectedTerminalTab).toHaveFocus();
  expect(keyboardSelectedTerminalTab).toHaveAttribute("aria-selected", "true");
  expect(keyboardSelectedTerminalTab).toContainElement(
    document.querySelector("[data-workflow-dock-active-indicator='dock']"),
  );
  expect(executionTab.querySelector("[data-workflow-dock-active-indicator]")).not.toBeInTheDocument();
  expect(useStore.getState().workflowDockWidth).toBe(418);
  expect(screen.getByRole("tabpanel")).toContainElement(document.querySelector("[data-terminal-runtime]"));
  await user.keyboard("{ArrowLeft}");
  expect(screen.getByRole("tab", { name: "执行" })).toHaveAttribute("aria-selected", "true");
  expect(screen.getByRole("tabpanel")).toContainElement(screen.getByRole("button", { name: "放大查看" }));
