...
test("model config adds a provider through the controlled value contract", async () => {
  ...
  renderWithFrontendProviders(React.createElement(ModelConfigHarness, {
...
test("provider discovery forwards the normalized endpoint without leaking empty headers", async () => {
  ...
  const user = userEvent.setup();
  renderWithFrontendProviders(React.createElement(ModelConfigView, createProps({
    value: {
...
function ModelConfigHarness({ initialValue, catalogs = {}, onChange }) {
  ...
  return React.createElement(ModelConfigView, createProps({
    value,
    catalogs,
    onChange: (next) => {
      setValue(next);
