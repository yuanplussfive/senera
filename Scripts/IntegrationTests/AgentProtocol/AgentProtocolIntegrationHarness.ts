...
export interface AgentProtocolIntegrationAuthentication {
...
export async function createAgentProtocolIntegrationHarness(
  handler: ScriptedLoopHandler = defaultScriptedLoopHandler,
  options: { authentication?: AgentProtocolIntegrationAuthentication; approvalRuntime?: AgentApprovalRuntime } = {},
): Promise<AgentProtocolIntegrationHarness> {
  const workspaceRoot = mkdtempSync(path.join(os.tmpdir(), "senera-e2e-"));
  const authentication = options.authentication;
  const config = createIntegrationConfig(await reserveTcpPort(), authentication);
  if (authentication) {
  ...
  sandboxRuntimeService.markUnavailable(new Error("Integration tests run without OS sandbox preparation."));
...
export class AgentProtocolIntegrationClient {
  ...
  static async connect(url: string, options: ClientOptions = {}): Promise<AgentProtocolIntegrationClient> {
    const socket = new WebSocket(url, options);
    await new Promise<void>((resolve, reject) => {
      socket.once("open", resolve);
      socket.once("error", reject);
    });
    return new AgentProtocolIntegrationClient(socket);
  }
  ...
  static async connectAuthenticated(input: {
    ...
    loginName: string;
    password: string;
  }): Promise<AgentProtocolIntegrationClient> {
...
function createIntegrationConfig(
  port: number,
  authentication?: AgentProtocolIntegrationAuthentication,
): AgentSystemConfig {
  return {
    Defaults: {
      Server: {
