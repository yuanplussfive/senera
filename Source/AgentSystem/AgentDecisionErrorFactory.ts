...
export class AgentDecisionErrorFactory {
  ...
  emptyDecisionXml(): AgentRetryableError {
    return this.createRetryable({
      ...
      diagnostics: [{
        message: "XML 输出为空。",
        pointer: "/",
        suggestion: this.allowedRootSuggestion(),
      }],
      details: {
        allowedRoots: this.allowedDecisionRoots(),
      },
    });
  }
  ...
  unknownDecisionRoot(options: {
    ...
    const suggestion = this.suggestionForUnknownDecisionRoot(options);
    const diagnostics = [
  ...
  private suggestionForUnknownDecisionRoot(options: {
    rootName: string;
    allowedRoots: string[];
  }): string {
    return [
  ...
  private allowedRootSuggestion(allowedRoots = this.allowedDecisionRoots()): string {
    return `把根标签改成允许的决策根标签之一：${allowedRoots.join(", ")}。`;
  }

  private allowedDecisionRoots(): string[] {
  ...
  invalidDecisionPayload(options: {
    ...
    const diagnostics = this.buildSchemaDiagnostics(
      options.source,
      options.rootName,
      [],
  ...
  unknownToolName(options: {
    ...
    const diagnostics = [
      options.source.diagnosticForPath(
        ...
        options.rootName,
        this.toolCallPath(options.protocol, options.callIndex, "name"),
  ...
  invalidToolArguments(options: {
    ...
    const diagnostics = this.buildSchemaDiagnostics(
      options.source,
      options.rootName,
      this.toolCallPath(options.protocol, options.callIndex, "arguments"),
      options.error.issues,
    );
  ...
  toolExecutionFailure(options: {
    ...
    const argumentsPath = this.toolCallPath(options.protocol, options.callIndex, "arguments");
    const diagnostics = this.remapToolDiagnostics(
      options.source,
      options.rootName,
      argumentsPath,
  ...
  private buildSchemaDiagnostics(
    source: AgentXmlSourceHelper,
    rootName: string,
