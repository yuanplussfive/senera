...
## 简述
按 `uploadUri` 统一处理用户上传文档。工具会先登记并探测文件事实；默认 `auto` 模式会在插件配置匹配到可解析格式时继续抽取正文、Markdown 预览、metadata、warnings 和 chunks。

## 何时使用
用户上传 PDF、Word、PowerPoint、Excel、OpenDocument、RTF、CSV、Markdown、HTML 等文件，并要求读取、总结、分析、提取要点或确认附件类型时使用。
...
## mode
- `auto`：先探测；匹配到插件配置的解析器时自动抽取正文。
- `probe`：只返回 MIME、编码、ZIP/OOXML 容器等探测事实。
- `extract`：要求抽取正文；如果没有匹配解析器会返回工具错误。
