...
export class AgentXmlSyntaxGuard {
  ...
  assertSafe(xmlText: string, sourceHelper: AgentXmlSourceHelper): void {
    ...
    const forbidden = new Map((this.policy?.forbiddenSyntaxRules ?? []).map((rule) => [rule.label, rule]));
    ...
    const failForbidden = (label: string) => {
      ...
      failure = new AgentXmlParseError(
        `XML 使用了禁止语法：${label}。`,
        [
          new AgentSourceDiagnosticBuilder(xmlText).fromPosition(`XML 使用了禁止语法：${label}。`, position, {
            suggestion:
              "删除 DOCTYPE、ENTITY、namespace 或处理指令；参数文本中的特殊字符请保持为普通文本或使用 XML 实体转义。",
          }),
        ],
  ...
  private assertClosedCdataSections(xmlText: string, sourceHelper: AgentXmlSourceHelper): void {
  ...
  private syntaxError(error: Error, sourceHelper: AgentXmlSourceHelper, parser: SaxesParser): AgentXmlParseError {
    const message = error.message.includes("unclosed cdata")
      ? "XML 中存在未闭合的 CDATA。"
      : `XML 格式无效：${error.message}`;
    const reason = error.message.includes("unclosed cdata") ? "unclosed_cdata" : "saxes_error";
    const position = Math.max(0, parser.position);
