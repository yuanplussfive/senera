...
import { createSettingsDiagnostics, groupSettingsSectionResults, readSettingsSectionPlan, readSettingsSectionStatus, readSettingsSectionGroup, readSettingsWorkbenchSectionSummary, searchSettingsSectionResults, searchSettingsSections, } from "../../../Frontend/src/features/settings/settingsPresentation.ts";
...
describe("settingsPresentation", () => {
    ...
    it("creates diagnostics for the active app surface", () => {
        expect(createSettingsDiagnostics({
            appVersion: "1.2.3",
    ...
    it("returns every section for an empty search query", () => {
        expect(searchSettingsSections(settingsSections, "").map((section) => section.id)).toEqual(settingsSections.map((section) => section.id));
    });
    ...
    it("matches section groups without enabling planned sections", () => {
        ...
        expect(searchSettingsSectionResults(settingsSections, "规划中").map((result) => ({
    ...
    it("summarizes config-backed sections by shared draft state", () => {
        ...
        expect(modelService && readSettingsWorkbenchSectionSummary(modelService, {
            label: "未保存",
            state: "dirty",
        })).toMatchObject({
    ...
    it("summarizes skills separately from the shared config draft", () => {
        ...
        expect(skills && readSettingsWorkbenchSectionSummary(skills, {
            label: "需配置",
    ...
    it("keeps system config actions owned by the content workspace", () => {
        ...
        expect(system && readSettingsWorkbenchSectionSummary(system, {
            label: "已同步",
            state: "synced",
        })).toMatchObject({
            actionKind: "config",
