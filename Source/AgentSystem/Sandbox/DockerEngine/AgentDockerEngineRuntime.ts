...
import type { AgentSandboxExecutionRequest } from "../Worker/AgentSandboxWorkerProtocol.js";
import {
  readAgentDockerEngineRuntimeContract,
  type AgentDockerEngineSandboxProvider,
  type ResolvedAgentDockerEngineRuntimeContract,
} from "../DockerEngine/AgentDockerEngineRuntimeContract.js";
...
export interface AgentDockerProcessExit {
  exitCode: number | null;
  signal: string | null;
}

export interface AgentDockerProcess {
  readonly id: string;
  readonly stdin: Duplex;
  readonly stdout: Readable;
  readonly stderr: Readable;
  readonly completion: Promise<AgentDockerProcessExit>;
...
export interface AgentDockerRuntime {
  ...
  start(request: AgentSandboxExecutionRequest): Promise<AgentDockerProcess>;
}
...
export async function resolveAgentDockerEngineSandboxProvider(input: {
  ...
  if (info.OSType !== "linux") {
    throw new Error(`Senera requires a Linux Docker Engine; connected daemon reports ${info.OSType ?? "unknown"}.`);
  }
  ...
  if (selected !== "gvisor" && selected !== "docker-engine") {
    throw new Error(`Docker Engine provider resolution selected incompatible provider ${selected}.`);
  }
  return { provider: selected, registeredRuntimes };
}

export class AgentDockerEngineRuntime implements AgentDockerRuntime {
  ...
  async prepare(
    ...
    this.prepared = false;
    input.onProgress?.({
      stage: AgentSandboxPreparationStages.DetectingEngine,
      item: this.runtimeName ?? "Docker Engine",
    });
    await this.assertEngineCompatible();
    await this.assertRuntimeStrategyAvailable();
    await this.prepareImage(input.onProgress);
    await this.runToolchainProbes(input.onProgress);
    this.prepared = true;
  }

  async start(request: AgentSandboxExecutionRequest): Promise<AgentDockerProcess> {
    if (!this.prepared) {
    ...
    try {
      ...
      const completion = (async (): Promise<AgentDockerProcessExit> => {
  ...
  private async prepareImage(report?: (progress: AgentSandboxPreparationProgress) => void): Promise<void> {
    const imagePresent = await this.validateImageIfPresent(this.imageReference);
    const shouldPull =
      this.options.pullPolicy === "always" || (!imagePresent && this.options.pullPolicy === "if-missing");
    if (shouldPull) await this.pullImage(report);
    if (!shouldPull && !imagePresent) {
      throw new Error(
        `Docker sandbox runtime image is unavailable under pull policy ${this.options.pullPolicy}: ${this.imageReference}.`,
      );
    }
    report?.({ stage: AgentSandboxPreparationStages.VerifyingImage, item: this.imageReference });
    if (!(await this.validateImageIfPresent(this.imageReference))) {
      throw new Error(`Docker sandbox runtime image is unavailable after preparation: ${this.imageReference}.`);
    }
  }

  private async pullImage(report?: (progress: AgentSandboxPreparationProgress) => void): Promise<void> {
    report?.({ stage: AgentSandboxPreparationStages.PullingImage, item: this.imageReference });
    let stream: NodeJS.ReadableStream;
    try {
      stream = await this.options.docker.pull(this.imageReference);
    } catch (error) {
      throw dockerOperationError("pull image", error);
    }
    const layers = new Map<string, { current: number; total?: number }>();
    await new Promise<void>((resolve, reject) => {
      dockerModem(this.options.docker).followProgress(
        stream,
        (error) => (error ? reject(dockerOperationError("pull image", error)) : resolve()),
        (event) => {
          const progress = dockerPullProgress(event);
          if (!progress) return;
          ...
          const totals = values.map((entry) => entry.total);
          report?.({
            stage: AgentSandboxPreparationStages.PullingImage,
            item: this.imageReference,
            downloadedBytes: values.reduce((sum, entry) => sum + entry.current, 0),
            ...(totals.every((value): value is number => value !== undefined)
              ? { totalBytes: totals.reduce((sum, value) => sum + value, 0) }
              : {}),
          });
        },
      );
  ...
  private async runToolchainProbes(report?: (progress: AgentSandboxPreparationProgress) => void): Promise<void> {
    const probes = this.resolvedContract.image.probes;
    for (const [index, probe] of probes.entries()) {
      report?.({
        stage: AgentSandboxPreparationStages.ProbingToolchain,
        item: probe.id,
        completed: index,
        total: probes.length,
      });
      await this.runProbeContainer(probe);
    }
    report?.({
      stage: AgentSandboxPreparationStages.ProbingToolchain,
  ...
  private async runProbeContainer(probe: { id: string; command: string; arguments: readonly string[] }): Promise<void> {
    const contract = this.resolvedContract.contract;
    ...
    try {
      ...
      if (result.StatusCode !== 0) {
        throw new Error(
          `${this.provider()} sandbox toolchain probe ${probe.id} exited with status ${String(result.StatusCode)}.`,
        );
      }
    } finally {
      await container.remove({ force: true }).catch(ignoreMissingDockerResource);
  ...
  private assertRequestPolicy(request: AgentSandboxExecutionRequest): void {
  ...
  private async stageRootfsCopies(copies: AgentSandboxExecutionRequest["rootfsCopies"]): Promise<string> {
...
function projectSandboxContainerEnvironment(
  environment: Readonly<Record<string, string>>,
  guestWorkspaceRoot: string,
): Record<string, string> {
  const projected = { ...environment };
  // Git sees a Windows bind mount as root-owned inside Linux. Senera owns this
  // reserved namespace so every container trusts only the declared workspace.
  for (const name of Object.keys(projected)) {
    const normalized = name.toUpperCase();
    if (normalized === "GIT_CONFIG_COUNT" || /^GIT_CONFIG_(?:KEY|VALUE)_[0-9]+$/u.test(normalized)) {
  ...
  return {
    ...
    GIT_CONFIG_VALUE_0: guestWorkspaceRoot,
  };
}
...
function dockerPullProgress(event: unknown): { id: string; current: number; total?: number } | undefined {
  if (!isRecord(event) || typeof event.id !== "string" || !isRecord(event.progressDetail)) return undefined;
