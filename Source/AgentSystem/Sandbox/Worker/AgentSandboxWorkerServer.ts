...
import type { AgentDockerProcess, AgentDockerRuntime } from "../DockerEngine/AgentDockerEngineRuntime.js";
import { readAgentDockerEngineRuntimePolicyContract } from "../DockerEngine/AgentDockerEngineRuntimeContract.js";

export interface AgentSandboxWorkerServerOptions {
  socketPath: string;
...
export class AgentSandboxWorkerServer {
  private readonly server: net.Server;
  private readonly maxConcurrentExecutions: number;
  private activeExecutions = 0;
  private preparation: Promise<void> | undefined;

  constructor(private readonly options: AgentSandboxWorkerServerOptions) {
    const contract = readAgentDockerEngineRuntimePolicyContract("docker-engine");
  ...
  async start(): Promise<void> {
    await prepareWorkerEndpoint(this.options.socketPath);
    this.server.listen(this.options.socketPath);
    try {
      await once(this.server, "listening");
      if (process.platform !== "win32") await chmod(this.options.socketPath, this.options.socketMode ?? 0o600);
    } catch (error) {
  ...
  async close(): Promise<void> {
    ...
    if (process.platform !== "win32") await unlink(this.options.socketPath).catch(ignoreMissingFile);
  }
  ...
  private async handleExecution(
    socket: net.Socket,
    decoder: AgentSandboxWorkerFrameDecoder,
    message: Extract<AgentSandboxWorkerClientMessage, { type: "start" }>,
  ): Promise<void> {
...
async function readFirstMessage(
  socket: net.Socket,
  decoder: AgentSandboxWorkerFrameDecoder,
): Promise<AgentSandboxWorkerClientMessage> {
  for await (const chunk of socket.iterator({ destroyOnReturn: false })) {
    ...
    return AgentSandboxWorkerClientMessageSchema.parse(frames[0]);
  }
...
async function consumeExecutionMessages(
  socket: net.Socket,
  decoder: AgentSandboxWorkerFrameDecoder,
  process: AgentDockerProcess,
): Promise<void> {
  for await (const chunk of socket) {
    for (const frame of decoder.push(chunk)) {
      const message = AgentSandboxWorkerClientMessageSchema.parse(frame);
...
function forwardOutput(output: NodeJS.ReadableStream, socket: net.Socket, stream: "stdout" | "stderr"): void {
  output.on("data", (chunk: Buffer | string) => {
    ...
    const accepted = socket.write(
      encodeAgentSandboxWorkerFrame({ type: "output", stream, data: data.toString("base64") }),
    );
...
async function writeFrame(socket: net.Socket, message: AgentSandboxWorkerServerMessage): Promise<void> {
  if (socket.write(encodeAgentSandboxWorkerFrame(message))) return;
...
async function prepareWorkerEndpoint(socketPath: string): Promise<void> {
  if (process.platform === "win32") {
    if (!socketPath.startsWith("\\\\.\\pipe\\")) {
      throw new Error("Windows sandbox worker endpoints must use a named pipe path.");
    }
    return;
  }
  if (!path.isAbsolute(socketPath)) throw new Error("Sandbox worker socket path must be absolute.");
...
function resolveWorkerError(error: unknown): { code: string; message: string } {
  if (error instanceof Error) {
    const code = "code" in error && typeof error.code === "string" ? error.code : "sandbox_worker_failed";
    return { code, message: error.message };
  }
  return { code: "sandbox_worker_failed", message: String(error) };
}
