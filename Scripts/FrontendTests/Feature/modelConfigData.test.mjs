...
import { applyModelProvidersDraft, filterRemoteModelPickerRows, readProviderModelRows, remoteModelCategories, remoteModelCategoryMatches, } from "../../../Frontend/src/features/chat/modelConfigData.ts";
describe("remote model picker helpers", () => {
    ...
    it("combines category filtering with search text", () => {
        expect(filterRemoteModelPickerRows({
            rows,
            category: "embedding",
            search: "small",
        }).map((row) => row.id)).toEqual(["text-embedding-3-small"]);
        expect(filterRemoteModelPickerRows({
            rows,
            category: "all",
            search: "openai",
    ...
    it("keeps configured local rows available when the remote catalog is empty", () => {
        expect(readProviderModelRows({
            catalogModels: [],
            configuredOnly: false,
            providerId: "local",
            search: "",
            models: [
                { Id: "local/local-only", ProviderId: "local", Endpoint: "chat", Model: "local-only" },
            ],
        })).toEqual([{ id: "local-only", ownedBy: "local" }]);
    });
    it("keeps an existing default model when applying model provider drafts", () => {
        expect(applyModelProvidersDraft({
    ...
    it("falls back when the requested default model is removed", () => {
        ...
        }).DefaultModelProviderId).toBe("openai/gpt-4.1-mini");
        expect(applyModelProvidersDraft({
            requestedDefaultModelId: "openai/missing",
            value: {
                DefaultModelProviderId: "openai/missing",
