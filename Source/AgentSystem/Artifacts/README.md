...
Workspace checkpoints publish byte length and SHA-256 identities for both snapshot JSON files. `WorkspaceRestoreCheckpoint` verifies those identities and the manifest path binding before reading either side, derives the expected live revision from the opposite snapshot, and checks the live workspace again after all Artifact reads. Restore mutations are protected by an exclusive whole-workspace resource claim and keep a bounded per-path rollback backup, so a cancellation or late filesystem error cannot leave an earlier subset of the restore applied. Paths outside the captured current snapshot are never deleted implicitly; an entry that appeared there is reported as a revision conflict.
