...
test("decision events project prompt, route, planner stage, and fallback plan state", () => {
  ...
  applyEvent(state, createEvent(EventKinds.PromptSummary, {
    chars: 1200,
    lines: 42,
    tokenCount: 330,
  }, { step: 1, sequence: 2, phase: "prompt" }));
  applyEvent(state, createEvent(EventKinds.InteractionRouted, {
  ...
  applyEvent(state, createEvent(EventKinds.ActionPlannerStageStarted, {
    stage: "understandUserTurn",
  }, { step: 1, sequence: 4, phase: "decision" }));
  applyEvent(state, createEvent(EventKinds.ActionPlannerStageCompleted, {
    stage: "understandUserTurn",
  ...
  applyEvent(state, createEvent(EventKinds.ActionPlanned, {
    status: "fallback",
    preferredTools: [],
    toolSearchQueries: [],
    loadedTools: [],
...
test("model stream events keep visible answer text while closing the model step", () => {
  ...
  applyEvent(state, createEvent(EventKinds.ModelStarted, {
  ...
  applyEvent(state, createEvent(EventKinds.ModelDelta, {
    text: "第一段",
  }, { step: 1, sequence: 3, phase: "model" }));
  applyEvent(state, createEvent(EventKinds.ModelDelta, {
    text: "，第二段。",
  }, { step: 1, sequence: 4, phase: "model" }));
  applyEvent(state, createEvent(EventKinds.ModelCompleted, {
    text: "第一段，第二段。",
...
test("pi trace lifecycle merges started and completed events into one scoped step", () => {
  ...
  applyEvent(state, createEvent(EventKinds.PiTrace, {
    source: "session",
