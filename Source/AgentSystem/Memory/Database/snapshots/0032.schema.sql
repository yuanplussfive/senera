...
create table "continuity_turn_value_examples" ( prompt_hash text primary key , occurrences integer not null check ( occurrences >= 0 ) , first_seen_at text not null , last_seen_at text not null , prompt_text text not null default '' , label text not null default 'unproductive' check ( label in ( 'valuable' , 'unproductive' ) ) );
...
-- index: idx_continuity_turn_value_examples_identity
create unique index idx_continuity_turn_value_examples_identity on continuity_turn_value_examples ( prompt_hash , label );
