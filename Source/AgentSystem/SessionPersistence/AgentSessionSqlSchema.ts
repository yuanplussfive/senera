...
const AgentSessionSchemaVersion = 3;

export function configureAgentSessionDatabase(db: Database.Database): void {
...
export function runAgentSessionMigrations(db: Database.Database): void {
  const row = db.pragma("user_version", { simple: true }) as number;
  if (row >= AgentSessionSchemaVersion) {
    return;
  }

  db.pragma(`user_version = ${AgentSessionSchemaVersion}`);
}
