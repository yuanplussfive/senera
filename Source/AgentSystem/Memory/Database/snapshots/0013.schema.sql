...
-- table: agent_goals
create table agent_goals ( id text primary key , uri text not null unique , scope_kind text not null , scope_id text not null , objective text not null , status text not null check ( status in ( 'active' , 'paused' , 'blocked' , 'completed' , 'cancelled' ) ) , reason text , created_at text not null , updated_at text not null , completed_at text );
